...
\
For *Artinis Oxysoft* exports, which number their data table columns, channel names are read automatically from the file *Legend* metadata: named traces become `nirs_channels` (non-alphanumeric characters replaced with underscores, e.g. `Rx1_Tx1_O2Hb`), the *(Sample number)* column becomes `sample` (with a `time` column derived from the export sample rate), the *(Event)* column becomes `event`, and a trailing un-numbered event label column becomes `labels`. Explicit channel arguments override this detection.\
\
