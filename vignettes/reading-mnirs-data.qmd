...
* `data`
    ...
    This function takes in a data frame, detects and replaces outliers, invalid values, and missing values across the specified channels, then returns the processed data frame. *`mnirs`* metadata will be passed to and from this function.
...
* `nirs_channels`
    ...
    Specify which column names in `data` to process. If the data contain *`mnirs`* metadata, these channels will be detected automatically. Channels not specified will be passed through unprocessed to the returned data frame.
